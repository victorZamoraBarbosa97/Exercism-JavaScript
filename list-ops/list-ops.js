export class List {
  ...
  concat(lists) {
    ...
    return lists.eachWithObject(new List(), (all, list) => all.append(list));
  }

  filter(filterFunc) {
    return this.eachWithObject(
      new List(),
      (list, el) => filterFunc(el) && list.push(el)
    );
  }

  map(mapFunc) {
    return this.eachWithObject(new List(), (list, el) =>
      list.push(mapFunc(el))
    );
  }
  ...
  reverse() {
    return this.eachWithObject(new List(), (list, el) => list.unshift(el));
  }
